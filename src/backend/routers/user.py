...
async def self_get(user: Annotated[User, Depends(get_current_user)]) -> User:
    return user
...
async def self_delete(db: Annotated[Session, Depends(get_db)], user: Annotated[User, Depends(get_current_user)]) -> None:
